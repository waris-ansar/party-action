...
function Work({active, setActiveSection}) {
  return (
    <section
      id="first"
      className={`opacity-100 slide-in ${active === 'work' && "active"}`}
    //   style={{ transform: "translate3d(100%, 0px, 0px)" }}
    >
      <div className="sec-header" onClick={() => setActiveSection('work')}>
        <span className="absolute top-9 left-0 right-0 text-center text-white text-2xl font-satoshi font-medium">01</span>
        <span className="absolute bottom-9 text-white text-2xl font-satoshi font-medium text-header-menu">Work</span>
      </div>


      <div className="sec-body">
        <div className={`container justify-between flex-col ${active === "work" && "active"}`}>
